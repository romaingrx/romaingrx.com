const darkTheme = {
  '.romaingrx-dark': {
    '--romaingrx-colors-brand': 'hsla(var(--palette-pink-50), 100%)',
    '--romaingrx-colors-body': 'hsla(var(--palette-gray-80), 100%)',
    '--romaingrx-colors-header': 'hsla(var(--palette-gray-02), 40%)',
    '--romaingrx-colors-emphasis': 'hsla(var(--palette-blue-50), 5%)',
    '--romaingrx-colors-foreground': 'hsla(var(--palette-gray-00), 60%)',
    '--romaingrx-colors-danger': 'hsla(var(--palette-red-55), 100%)',
    '--romaingrx-colors-danger-emphasis': 'hsla(var(--palette-red-15), 15%)',
    '--romaingrx-colors-warning': 'hsla(var(--palette-orange-80), 100%)',
    '--romaingrx-colors-warning-emphasis':
      'hsla(var(--palette-orange-45), 15%)',
    '--romaingrx-colors-success': 'hsl(var(--palette-green-80))',
    '--romaingrx-colors-success-emphasis':
      'hsla(var(--palette-green-45), 15%)',
    '--romaingrx-colors-typeface-primary':
      'hsla(var(--palette-indigo-90), 100%)',
    '--romaingrx-colors-typeface-secondary':
      'hsla(var(--palette-gray-55), 100%)',
    '--romaingrx-colors-typeface-tertiary':
      'hsla(var(--palette-gray-60), 100%)',
    '--romaingrx-colors-typeface-danger':
      'hsla(var(--palette-red-60), 100%)',
    '--romaingrx-border-color': 'var(--romaingrx-colors-emphasis)',
    '--romaingrx-card-background-color': 'hsl(var(--palette-gray-01))',
    '--romaingrx-form-input-active': 'var(--romaingrx-colors-brand)',
    '--romaingrx-form-input-background': 'hsl(var(--palette-gray-00))',
    '--romaingrx-form-input-disabled': 'hsl(var(--palette-gray-09))',
    '--romaingrx-form-input-border': 'hsl(var(--palette-blue-10))',
    '--romaingrx-form-input-focus': 'hsla(var(--palette-blue-40), 0.55)',
    '--shadow-color': 'var(--local-shadow-color, 223deg 5% 80%)',
    '--code-snippet-background': 'hsla(var(--palette-gray-03), 0.2)',
    '--token-comment': 'var(--romaingrx-colors-typeface-tertiary)',
    '--token-selector': 'hsl(var(--palette-indigo-50))',
    '--token-symbol': 'hsl(var(--palette-blue-60))',
    '--token-operator': 'hsl(var(--palette-orange-75))',
    '--token-keyword': 'hsl(var(--palette-blue-50))',
    '--token-function': 'hsl(var(--palette-pink-60))',
    '--token-punctuation': 'hsl(var(--palette-indigo-60))',
  },
};

export default darkTheme;