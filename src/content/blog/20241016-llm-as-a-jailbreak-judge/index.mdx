...
permalink: llm-as-a-jailbreak-judge
...
import BlogLink from '@/components/blog/components/blog-link.astro';

# Setup

For this LLM-as-a-judge experiment, I curated the dataset created during the HarmBench evolution in the <BlogLink post="20240927-mistral-nemo-red-teamer">Mistral Nemo Red Teamer post</BlogLink> and decided to keep only the data from the completions of starling 7B as we achieved a good ASR so it's a good balance for the labels. I also decided to drop the copyright category as it has no meaning of jailbreak in terms of harmfulness. So in the end we have a dataset of 300 entries with 167 entries labeled as jailbroken.
